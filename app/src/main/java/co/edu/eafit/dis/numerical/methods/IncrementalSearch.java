...
public class IncrementalSearch {
  ...
  public double[] evaluate(double x0, double delta, double nIter)
    ...
    headers.addCell(c.getResources().getString(
        R.string.text_results_table_x_value));
    headers.addCell(c.getResources().getString(
        R.string.text_results_table_fxi_value));
    ResultsTable.addRow(headers);
    // interval contiene el menor, mayor del intervalo donde hay una ra?z.
    ...
    firstRow.addCell("0"); // Se agrega la primera iteraci?n (0)
    firstRow.addCell(String.valueOf(x0)); // Se agrega el valor inicial
    firstRow.addCell(String.valueOf(fx0)); // Se agrega el valor inicial
                                           // evaluado
    ...
    if (fx0 == 0.0) {
      ...
      if (fx1 == 0.0) {
        interval[0] = interval[1] = x1; // x1 una ra?z
        ...
        interval[0] = x0;
        interval[1] = x1;
        // Hay una ra?z desde x0 hasta x1
        ResultsTable.Row finalRow = results.new Row();
        finalRow.addCell(String.valueOf(currentIteration));
        finalRow.addCell(String.valueOf(x1));
        finalRow.addCell(String.valueOf(fx1));
